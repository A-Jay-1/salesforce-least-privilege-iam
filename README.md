...
Baseline sales access is low-risk and high-frequency — forcing it through PIM would add friction without a proportional security benefit. Not every role needs the same level of control; least privilege means matching control to risk, not applying maximum friction everywhere.
...
**The side-by-side contrast is the real evidence here:** the same mechanism (PIM for Groups) produces genuinely different runtime behavior depending on the policy attached to each group — proof the risk-based design isn't just documented, it's enforced.
...
Documenting what *wasn't* done, and why, is as important as documenting what was.
